...
class SelectKBest_:
  ...
  def fit(self, x_train: pd.DataFrame, y_train: Union[pd.DataFrame, pd.Series],
    ...
    for i in range(len(x_train.columns)):
      ...
      model = XGBRegressor(random_state=0).fit(x_train_sel, y_train)
      if model_type == "classifier":
        ...
        train_score.append(mean_squared_error(y_train.ravel(), model.predict(x_train_sel).ravel())*(-1))
        val_score.append(mean_squared_error(y_val, model.predict(x_val_sel))*(-1))
...
class Embedded:
  ...
  def predict(self, x_val: pd.DataFrame, y_val: Union[pd.DataFrame, pd.Series]):
    reg = XGBRegressor(random_state=0).fit(self.x_train, self.y_train)
    feature_sel = self.x_train.columns[list(self.model.get_support())]
    reg_ = XGBRegressor(random_state=0).fit(self.x_train[feature_sel], self.y_train)
...
class Wrapper:
  ...
  def fit(self, x_train: pd.DataFrame, y_train: Union[pd.DataFrame, pd.Series], 
          forward: bool=True, floating: bool=False, verbose: int=2, scoring="neg_mean_squared_error", cv: int=5, n_jobs=-1) -> List[str]:
    self.model = sfs(XGBRegressor(), k_features=(1, len(x_train.columns)), forward=forward, 
  ...
  def predict(self, x_val: pd.DataFrame, y_val: Union[pd.DataFrame, pd.Series]):
    reg = XGBRegressor().fit(self.x_train, self.y_train)
    feature_sel = list(self.model.k_feature_names_)
    reg_ = XGBRegressor().fit(self.x_train[feature_sel], self.y_train)
    print(f"before score: {mean_squared_error(reg.predict(x_val), y_val)}")
    print(f"after score: {mean_squared_error(reg_.predict(x_val), y_val)}")